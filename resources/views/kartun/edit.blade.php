	...
	<div class="container">
		<div class="col-md-12">
			<div class="panel panel-primary">
			  <div class="panel-heading">Edit Data Kartun
			  	<div class="panel-title pull-right"><a href="{{ url()->previous() }}">Back</a>
			  	</div>
			  </div>
			  <div class="panel-body">
			  	<form action="{{ route('ktn.update',$ktn->id) }}" method="post" >
			  		<input name="_method" type="hidden" value="PATCH">
        			{{ csrf_field() }}
			  		<div class="form-group {{ $errors->has('judul') ? ' has-error' : '' }}">
			  			<label class="control-label">Judul</label>	
			  			<input type="text" name="judul" class="form-control" value="{{ $ktn->judul }}"  required>
			  			@if ($errors->has('judul'))
                    ...
                    <div class="form-group {{ $errors->has('channel') ? ' has-error' : '' }}">
			  			...
			  			<input type="text" name="channel" class="form-control" value="{{ $ktn->channel }}"  required>
			  		...
			  		<div class="form-group {{ $errors->has('tokoh') ? ' has-error' : '' }}">
			  			...
			  			<input type="text" name="tokoh" class="form-control" value="{{ $ktn->tokoh }}"  required>
			  			@if ($errors->has('tokoh'))
                            <span class="help-block">
			  		...
			  		<div class="form-group {{ $errors->has('watak') ? ' has-error' : '' }}">
			  			...
			  			<input type="text" name="watak" class="form-control" value="{{ $ktn->watak }}"  required>
			  			@if ($errors->has('watak'))
                            <span class="help-block">
                    ...
                    <div class="form-group {{ $errors->has('singkat_cerita') ? ' has-error' : '' }}">
			  			...
			  			<input type="text" name="singkat_cerita" class="form-control" value="{{ $ktn->singkat_cerita }}"  required>
